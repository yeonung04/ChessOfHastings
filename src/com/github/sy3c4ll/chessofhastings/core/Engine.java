...
import com.github.sy3c4ll.chessofhastings.pieces.Rules;
...
public class Engine implements ActionListener,Rules{
	...
	public boolean valid(Piece init,Piece dest){
		if(init==dest||init.getType()==PieceType.EMPTY)return false;
	...
	public void victory(Side s){
		for(int i=0;i<8;i++)for(int j=0;j<8;j++)b.b[i][j].getButton().setEnabled(false);b.k[0].getButton().setEnabled(false);b.k[1].getButton().setEnabled(false);
	...
	public void move(Piece init,Piece dest){
		...
		b.turn=(b.turn==Side.NORMAN?Side.SAXON:Side.NORMAN);b.l.setText(b.turn==Side.NORMAN?"Normans' Turn!":"Saxons' Turn!");;
		clear();
	}
	...
	@Override public void actionPerformed(ActionEvent e){
		...
		if(s==b.n){
			for(int i=0;i<8;i++)for(int j=0;j<8;j++)b.b[i][j].getButton().setEnabled(true);b.k[0].getButton().setEnabled(true);b.k[1].getButton().setEnabled(true);
			b.turn=Side.NORMAN;
